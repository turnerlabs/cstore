...
// GetBy ...
func GetBy(name, defaultVault string, clog catalog.Catalog, fileEntry *catalog.File, uo cfg.UserOptions, io models.IO) (contract.IVault, error) {
	if len(name) == 0 {
		v := vaults[defaultVault]
		return v, v.Pre(clog, fileEntry, uo, io)
	}
