...
func Select(file *catalog.File, clog catalog.Catalog, v contract.IVault, prompt bool, io models.IO) (contract.IStore, error) {

	if len(file.Store) > 0 {
		if store, found := stores[file.Store]; found {
			return store, store.Pre(clog, file, v, prompt, io)
		}
